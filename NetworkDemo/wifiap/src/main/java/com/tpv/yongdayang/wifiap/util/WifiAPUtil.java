package com.tpv.yongdayang.wifiap.util;
...
public class WifiAPUtil {
	...
	//监听wifi热点的状态变化
    public static final String WIFI_AP_STATE_CHANGED_ACTION = "android.net.wifi.WIFI_AP_STATE_CHANGED"; //该Action被隐藏
	...
	//打开移动热点
	public boolean OpenWifiAp(String str, String password,WifiSecurityType Type) {
		String ssid = str;
		//配置热点信息。
		...
		if(Type == WifiSecurityType.WIFICIPHER_NOPASS) { //网络开放，无密码
			if(DEBUG)Log.d(TAG, "wifi ap----no password");
			wcfg.allowedAuthAlgorithms.set(WifiConfiguration.AuthAlgorithm.OPEN, true); //设置为开放
			wcfg.wepKeys[0] = "";
			wcfg.allowedKeyManagement.set(KeyMgmt.NONE);
			wcfg.wepTxKeyIndex = 0;
		} else if(Type == WifiSecurityType.WIFICIPHER_WPA) { //WPA
		...
		} else if(Type == WifiSecurityType.WIFICIPHER_WPA2) {  //WPA2
