package com.tpv.yongdayang.networkdemo.util;
...
public class WifiUtil {
    ...
    public void openWifi(Context context) {
        ...
        }else{  //WifiManager.WIFI_STATE_ENABLED
    ...
    public StringBuilder lookUpScan() {
        ...
        for (int i = 0; i < mWifiList.size(); i++) {
            stringBuilder.append("Index_" + new Integer(i + 1).toString() + ":");// 将ScanResult信息转换成一个字符串包
    ...
    public WifiConfiguration CreateWifiInfo(String SSID, String Password, int Type)
    {
        ...
        {
            config.preSharedKey = "\""+Password+"\"";     //wifi密码
            config.hiddenSSID = true;
            config.allowedAuthAlgorithms.set(WifiConfiguration.AuthAlgorithm.OPEN); //身份验证协议
            config.allowedGroupCiphers.set(WifiConfiguration.GroupCipher.TKIP); //组密码
            config.allowedKeyManagement.set(WifiConfiguration.KeyMgmt.WPA_PSK); //密钥管理
            config.allowedPairwiseCiphers.set(WifiConfiguration.PairwiseCipher.TKIP); //WPA配对密码
